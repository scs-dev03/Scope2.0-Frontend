...
export class AdminBulkUploadComponent {
  ...
  this.utilitiesService.getLocations({dealer_id:this.mlForm.value.dealer}).subscribe((res:any)=>{
   ...
   getUploadedData(){
    ...
    this.globalBlockUiService.startLoading();
    this.stockUploadServiceBySCSUser.getUploadedData({dealer_id:this.mlForm.value.dealer,date:this.mlForm.value.date,user_id:this.userId}).subscribe((blob:any)=>{
