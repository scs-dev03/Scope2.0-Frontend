...
@Component({
  ...
  styleUrl: './multi-upload.component.css'
})
 ...
 isDataPresentPartNotInMaster:boolean=false;
  ...
  this.locations.forEach((loc:any) => {
    this.locationControls.push(
      this.fb.group({
        location: [loc.location_id, Validators.required],
        file: [null, ]
      })

      
    );
    this.fileUploadVisible.push(true);
  });
}


getLocationName(id: number): string {
  const loc = this.locations.find((l:any) => l.location_id === id);
  ...
  if (file) {
    this.locationControls.at(index).get('file')?.setValue(file);
    this.locationControls.at(index).get('file')?.markAsTouched();
  }
}
  ...
  const previous = this.previousLocations[index];

  if (previous && previous !== selected) {
    this.locationSelected.delete(previous);
  }

  this.previousLocations[index] = selected;

  if (this.locationSelected.has(selected)) {
    control?.setErrors({ duplicateLocation: true });
  } else {
    this.locationSelected.add(selected);
  }
}
 ...
 validateLocations() {
  ...
  if (!filesSelected) {
    this.messageService.add({
      ...
      detail: 'Please upload at least one file before submitting.',
      life: 4000
    });
    return;
  }
...
this.formData.append('brand_id', this.brandId.toString());
  ...
  // this.http.post('your-endpoint', this.formData).subscribe(...);
        ...
        this.tocService.uploadToc(this.formData).subscribe((res:any)=>{
         ...
         //    console.log("item ", item);
          ...
          this.formData=new FormData();
          this.clearFileUploads();
          // this.mlForm?.get('locations')?.reset();
        ...
        Object.keys(this.mlForm.controls).forEach((control:any)=>{
          this.mlForm.get(control)?.markAsTouched();
        })
 ...
 if (this.fu && this.fu.toArray().length > 0 && this.locationControls.controls.length > 0) {
  ...
  this.fileUploadVisible[index] = false;

    setTimeout(() => {
      this.fileUploadVisible[index] = true;
    }, 10); // Force re-render in next change detection cycle
 ...
 this.fileUploadVisible[index] = false;
    ...
    getRecords(){
      ...
      this.globalBlockUiService.startLoading();
      const allLocations = this.mlForm.get('locations')?.value;
...
const filteredLocations = allLocations
    ...
    locationIds.push({locationId:filteredLocations[i].location});
      ...
      this.records = this.records.flat().map((item: any) => {
        ...
        dealerName:dealerObj?.dealer_name,
          added_on: (item.addedOn),  // Format the added_on date
  ...
  this.onLocationChange(index); // optional: for duplicate validation
}
   ...
   // console.log(brandObj,dealerObj,this.mlForm.value.brand,this.mlForm.value.dealer)
