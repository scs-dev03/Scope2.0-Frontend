...
export class SingleLocationComponent {
 @ViewChild('dataTable') dataTable: Table | undefined;
  ...
  this.dealerId=localStorage.getItem('dealerid');
   this.brandId=localStorage.getItem('brandid')?.toString();
 ...
 // this.dealerId=20210;
  ...
  // this.getBrands();  
   ...
   this.sidebarService.visibleSidebar$.subscribe((visible:any)=>{
    ...
    this.sharedService.updateModuleName('Single Location TOC Upload');
  ...
  onSelect(event: any) {
   ...
   // this.userId=1;
       ...
       formData.append('updatedBy', this.userName.toString());
        formData.append('brand_id', this.brandId.toString());
         formData.append('dealer_id', this.dealerId.toString());
       this.globalBlockUiService.startLoading();
      this.tocUploadService.uploadToc(formData).subscribe((res:any)=>{
      //  this.getPartNotInMaster();
       ...
       if(res?.status)
       {
        this.messageService.add({severity:'error',life:8000,summary:res?.log});
       }else{
        this.showTable=true;
       }
        ...
        if(this.showTable){
         ...
         if(!res?.status)
       {
        this.messageService.add({severity:'success',life:4000,summary:'TOC uploaded Successfully!'});
       }
          // if(res?.allPartsNotInMaster!=0){
  ...
  //  }
   ...
   // this.userId=1;
    ...
    this.tocUploadService.getRecords({locations:[{locationId:this.slForm.value.location}],dealerId:this.dealerId}).subscribe((res:any)=>{
